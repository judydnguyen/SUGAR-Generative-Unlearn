...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            print("Error loading image w index: ", idx)
            img = Image.open(os.path.join(self.path, self.imgs[idx-1])).convert("RGB")
            # return None
        # img = Image.open(os.path.join(self.path, self.imgs[idx])).convert("RGB")
        if self.transform:
            img = self.transform(img)
...
def calc_fid_two_paths(fake_image_pretrained_path, fake_image_ours_path, batch_size=50):
    inception = InceptionV3([InceptionV3.BLOCK_INDEX_BY_DIM[2048]]).cuda()
    inception.eval()
    transform = transforms.ToTensor()

    num_samples = len(os.listdir(fake_image_pretrained_path))
    print("Num samples: ", num_samples)
    
    source_data = ImageDataset(fake_image_pretrained_path, transform=transform, num_samples=num_samples)
    source_loader = DataLoader(source_data, batch_size=batch_size, shuffle=False, num_workers=54, drop_last=False)

    print(f"Loaded fake_image_pretrained_path: {fake_image_pretrained_path}")
    print(f"Loaded fake_image_ours_path: {fake_image_ours_path}")

    
    our_data = ImageDataset(fake_image_ours_path, transform=transform, num_samples=num_samples)
    our_loader = DataLoader(our_data, batch_size=batch_size, shuffle=False, num_workers=54, drop_last=False)
    ...
    return fid_real, fid_real
    

@click.command()
@click.option("--pretrained_ckpt", type=str, default="../ffhqrebalanced512-128.pkl")
@click.option("--unlearn_model_path", type=str, default=None)
...
def main(pretrained_ckpt, unlearn_model_path, exp, num_samples, batch_size, seed, angle_p, angle_y_abs, truncation, truncation_cutoff, **kwargs):
    if seed is not None:
    ...
    fake_image_ours_path = f"experiments/fake_images/{exp}"
    fake_image_pretrained_path = f"experiments/fake_images/pretrained/{seed}"
    
    output_file = kwargs.get("logging_file", None)
    inversion_image_path = kwargs.get("inversion_image_path", None)
    print(f"\nInversion Image Path: {inversion_image_path}")
    
    if inversion_image_path is not None:
        # inference with a specific subset
    ...
    # print(f"fake_image_ours_path: {fake_image_ours_path}")
    # print(f"fake_image_pretrained_path: {fake_image_pretrained_path}")
    skip_pretrained = False
    skip_unlearned = False
    # exit(1)
    ...
    if os.path.exists(fake_image_pretrained_path) and len(os.listdir(fake_image_pretrained_path)) == num_samples:
        print(f"Skipping generation from unlearned model as it already exists")
        skip_unlearned = True
    ...
    print(f"ckpt loaded: {unlearn_model_path}")
    if unlearn_model_path is None:
        unlearn_model_path = os.path.join("experiments", exp, "checkpoints", "last.pkl")
