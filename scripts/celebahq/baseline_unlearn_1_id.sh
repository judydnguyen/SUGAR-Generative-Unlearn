#!/bin/bash
# Script to run baseline unlearning experiment for 1 CelebAHQ identity

set -e  # Exit on error

# Configuration
GPU_ID=0
EXP_NAME="baseline_celebahq-forget-1"

# Run baseline unlearning experiment
CUDA_VISIBLE_DEVICES=${GPU_ID} python optimized_unlearn.py \
    --exp "${EXP_NAME}" \
    --inversion goae \
    --inversion_image_path data/exp01/celebahq-forget-1 \
