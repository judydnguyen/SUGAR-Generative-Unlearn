...
def unlearn(*args, **kwargs):
    ...
    suffix_image_path = inversion_image_path.split("/")[-1].split(".")[0]
    exp_dir = f"experiments/{exp}_baseline"
    ckpt_dir = f"experiments/{exp}_baseline/checkpoints"
    image_dir = f"experiments/{exp}_baseline/training/images/{suffix_image_path}"
    result_dir = f"experiments/{exp}_baseline/training/results/{suffix_image_path}"
    logging_dir = f"experiments/{exp}/logs"
        ...
        if inversion is None: # for random
            ...
            if os.path.isdir(inversion_image_path): # for OOD
                ...
                for view, angle_y in enumerate(np.linspace(-angle_y_abs, angle_y_abs, sample_views)):
                    ...
                    img_u = generator.synthesis(w_u, camera_params_view)["image"]
                    img_u = tensor_to_image(img_u)
                    img_u.save(os.path.join(result_dir, f"img_{suffix_image_path}_unlearn_before_0_{view}.png"))
                del img_u
        generator.train()
    ...
    for i in pbar:
        ...
        writer.add_scalar("loss/total_local", loss_local.item(), i)
        writer.add_scalar("loss/total_adj", loss_adj.item(), i)
        writer.add_scalar("loss/total_global", loss_global.item(), i)
        ...
        if i % 100 == 0:
            with torch.no_grad():
                ...
                img_u_save.save(os.path.join(image_dir, f"img_{suffix_image_path}_img_u_{str(i).zfill(5)}.png"))

                img_target_save = g_source.synthesis(w_target, camera_params_front)["image"]
                img_target_save = tensor_to_image(img_target_save)
                img_target_save.save(os.path.join(image_dir, f"img_{suffix_image_path}_img_target_{str(i).zfill(5)}.png"))
                generator.train()
            del img_u_save, img_target_save
        # if i % 20 == 0:
        #     all_imgs, all_imgs_unlearned = [], []
        #     with torch.no_grad():
        #         generator.eval()
            ...
            if os.path.isdir(inversion_image_path): # for OOD
                for i in range(len(filenames)):
                    for view, angle_y in enumerate(np.linspace(-angle_y_abs, angle_y_abs, sample_views)):
                        ...
                        img_origin = generator.synthesis(w_origin[[i]], camera_params_view)["image"]
                        img_origin = tensor_to_image(img_origin)
                        img_origin.save(os.path.join(result_dir, f"img_{suffix_image_path}_unlearn_after_{i}_{view}.png"))
            else: # for InD
                for view, angle_y in enumerate(np.linspace(-angle_y_abs, angle_y_abs, sample_views)):
                    cam2world_pose_view = LookAtPoseSampler.sample(np.pi / 2 + angle_y, np.pi / 2 + angle_p, cam_pivot, radius=cam_radius, device=device)
                    camera_params_view = torch.cat([cam2world_pose_view.reshape(-1, 16), intrinsics.reshape(-1, 9)], dim=1)
                    img_u = generator.synthesis(w_u, camera_params_view)["image"]
                    img_u = tensor_to_image(img_u)
                    img_u.save(os.path.join(result_dir, f"img_{suffix_image_path}_unlearn_after_0_{view}.png"))
        generator.train()

    snapshot_data = dict()
    snapshot_data["G_ema"] = copy.deepcopy(generator).eval().requires_grad_(False).cpu()
    print(f"Saving checkpoint to {os.path.join(ckpt_dir, f'img_{suffix_image_path}_last.pkl')}")
    with open(os.path.join(ckpt_dir, f"img_{suffix_image_path}_last.pkl"), "wb") as f:
