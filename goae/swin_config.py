...
def get_parser():
    class Args():
        def __init__(self):
            self.cfg = 'goae/swinv2.yaml'
            self.data = '../example/real_person'
            self.G_ckpt = '../pretrained/ffhqrebalanced512-128.pkl'
            self.E_ckpt = '../pretrained/encoder_FFHQ.pt'
            self.AFA_ckpt = '../pretrained/afa_FFHQ.pt'
            self.outdir = '../output/'
            self.cuda = '0'
            self.mlp_layer = 2
...
def get_config():
    ...
    if GLOBAL_PATH:
        ...
        args.cfg = os.path.join(GLOBAL_PATH, args.cfg)
